...
description: 'Fix namespace-envelope gaps as config-as-data with the cub-namespace CLI — stamp pod-security defaults on a Namespace Unit (apply-envelope) and clone a base envelope Space''s Units into an existing Space, re-homed with set-namespace (backfill). Use for "add pod-security labels to namespace X", "backfill the base envelope into apptique-prod", "bring this namespace up to the envelope standard", "fix the missing-pod-security findings". Dry-run by default; requires --commit --change-desc. Not for read-only checks (use namespace-audit / namespace-findings) or enforcement Triggers (a later skill).'
phase: act
...
- **`backfill`** — clones a base envelope Space's Units into an existing Space and re-homes them with `set-namespace`. What the base defines is the base's business; backfill clones and re-homes without reading the policies.
...
- "Backfill the base envelope into apptique-prod." → `backfill --space <s> --from <base> --namespace <ns>`.
...
3. **Commit** with `--commit --change-desc` (compose the description: summary, verbatim user prompt, condensed clarifications):
   ...
   cub-namespace backfill --space apptique-prod --from namespace-envelope --namespace apptique --commit \
     --change-desc "Backfill the base envelope. User prompt: ..."
   ```
