...
// Package cub is cub-namespace's gateway to ConfigHub. It builds an
// authenticated API client from the ambient cub session — the same session
// `cub auth login` creates, or the credentials cub passes to a plugin via the
// environment — and exposes the authentication preflight every
// ConfigHub-touching command runs first.
//
...
func Client(ctx context.Context) (*cubapi.Client, error) {
	clientOnce.Do(func() {
		client, clientErr = cubapi.ResolveClient(ctx, cubapi.ClientOptions{UserAgent: "cub-namespace"})
	})
