...
// Package cub is cub-workload's gateway to ConfigHub. It builds an authenticated
// API client from the ambient cub session — the same session `cub auth login`
...
func Client(ctx context.Context) (*cubapi.Client, error) {
	clientOnce.Do(func() {
		client, clientErr = cubapi.ResolveClient(ctx, cubapi.ClientOptions{UserAgent: "cub-workload"})
	})
