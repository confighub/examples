...
readiness fix or a new PDB authored in a base Space flows to the environment and
cluster Spaces cloned from it, keeping each Space's local customizations.

Scope to the Units you mean with --where or a label shorthand (e.g. --component
checkout, --environment staging). Dry run unless --commit --change-desc; never
bypasses ApplyGates.`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			changeDesc, dryRun, err := commit.Validate("promote workload Units from upstream")
			if err != nil {
				return err
			}
