...
Per-workload readiness ("which workloads run as root / have no limits / no
probes?") is reported by the 'readiness' command; this is raw inventory only.`,
		Args: cobra.NoArgs,
...
func buildSnapshotReport(snap *snapshot.Snapshot) snapshotReport {
	...
	// workload-relevant config (i.e. appear in snap.Clusters); this naturally
