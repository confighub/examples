...
// Package cub is cub-rbac's gateway to ConfigHub. It builds an authenticated API
...
// github.com/confighub/sdk/core/cubapi package; cub-rbac no longer shells out to
// the cub CLI.
package cub
...
func Client(ctx context.Context) (*cubapi.Client, error) {
	clientOnce.Do(func() {
		client, clientErr = cubapi.ResolveClient(ctx, cubapi.ClientOptions{UserAgent: "cub-rbac"})
	})
