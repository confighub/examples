# Managing a VM Fleet with ConfigHub — ARCHIVED

> **This example no longer works and is retained for reference only.**
>
> It is built on `cub unit apply` and `cub unit destroy`, and on installing a
> bridge worker into the cluster — all removed from ConfigHub. `bin/setup`,
> `bin/setup-cluster`, `bin/start`, and `bin/stop` will fail.
>
> A rewrite onto the release model was attempted and abandoned: the example
> keeps every unit in one space separated by label, and a Release bundles every
> unit in a space, so the `instance-base` template would be deployed as a real
> autoscaling group. Making it work means restructuring the example into
> separate base and deployment spaces and reworking the scripts — rebuilding it
> rather than updating it.
>
> Its documentation page at `docs.confighub.com/get-started/examples/vm-fleet/`
> has been removed; the full text is inlined below so it is preserved alongside
> the scripts and base config.
>
> For the current model — components, deployment variants, and publishing
> immutable OCI Releases that a GitOps operator pulls — start with the
> [tutorial](https://docs.confighub.com/get-started/tutorial/) and
...
Make sure you have completed the [prerequisites](https://docs.confighub.com/get-started/examples/index/#prerequisites), then get the scripts and files ready with:

    git clone https://github.com/confighub/examples.git
    cd examples/vm-fleet
...
The `setup-iam` script will use your AWS account to create a new IAM user and grant the permissions necessecary for this example to work. Please consult [aws/policy.json](https://github.com/confighub/examples/blob/main/vm-fleet/aws/policy.json) to review what permissions are being granted.
...
ConfigHub doesn't use variables. Instead information such as network identifiers are "propagated" between units using the [Needs/Provides](https://docs.confighub.com/background/concepts/needsprovides/) mechanism and written literally into the config resource data. This mechanism currently does not support propagation in this particular case so we use a script instead. But in the future, the links between units will be used to propagate the values automatically.
