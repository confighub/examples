...
> **Note:** This example predates the ConfigHub component/variant model and uses the older flat-unit, clone-based workflow (including `cub helm install`/`cub helm upgrade` behavior that has since changed). It has been archived. The full instructions below are preserved as they were; see the current [Helm Charts guide](https://docs.confighub.com/guide/helm-charts/) for the up-to-date `cub helm` workflow.
...
Make sure you have completed the [prerequisites](https://docs.confighub.com/get-started/examples/#prerequisites), then get the scripts and files ready with:
