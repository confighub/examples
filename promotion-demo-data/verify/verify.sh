...
#
# It lives in its own folder so the stable example's scripts stay unchanged.
#
# Usage:
...
Safe next steps:
  cd .. && ./setup.sh
  cd ../promotion-demo-data/verify && ./verify.sh
  ./verify.sh --json
EOF
}
  ...
  cat <<'EOF'
{
  ...
  "depends_on_example": "..",
  "setup_command": "cd .. && ./setup.sh",
  "cleanup_command": "cd .. && ./cleanup.sh",
  "checks": [
