...
const (
	defaultAutoscalePolicySpace = "autoscale-policy"
	autoscaleFilterSlug         = "autoscale-guardrails"
	autoscalePackLabel          = "autoscale-guardrails"
)

// celAutoscalerNotPinned passes unless an autoscaler is pinned (min == max, so it
// can't actually scale). vet-cel: the resource is 'r'. An HPA's maxReplicas is
// required and minReplicas defaults to 1; a ScaledObject's bounds are both
// optional, so an absent bound is treated as unpinned.
...
type guardrailTrigger struct {
	slug string
	desc string
	fn   string // ConfigHub validating function
	args []api.FunctionArgument
}

var guardrailTriggers = []guardrailTrigger{
	{"autoscaler-not-pinned",
		"Warns on an HPA/ScaledObject with min == max (it can't actually scale). Fix: `cub-autoscale set-hpa --min <lower> --max <higher>` or a profile.",
		"vet-cel",
		[]api.FunctionArgument{{ParameterName: "expression", Value: celAutoscalerNotPinned}}},
	{"schema-valid",
		"Warns when a resource fails Kubernetes/CRD schema validation — covers KEDA ScaledObjects produced by `cub-autoscale convert-keda` (keda.sh is in the schema catalog).",
		"vet-schemas",
		nil},
}
...
Triggers another way are reported, not modified.`,
	...
	cmd.Flags().StringVar(&policySpace, "policy-space", defaultAutoscalePolicySpace, "Space that holds the guardrail Triggers and Filter")
...
func buildGuardrailsPlan(ctx context.Context, client *cubapi.Client, policySpace, whereSpace string) (guardrailsPlan, error) {
	plan := guardrailsPlan{PolicySpace: policySpace, Filter: policySpace + "/" + autoscaleFilterSlug}
...
func executeGuardrails(cmd *cobra.Command, client *cubapi.Client, policySpace string, plan guardrailsPlan) error {
	...
	ps, err := cubapi.EnsureSpace(ctx, client, goclientnew.Space{
		Slug:   policySpace,
		Labels: map[string]string{"app": "autoscale-manager", "role": "policy"},
	})
	if err != nil {
	...
	for _, t := range guardrailTriggers {
		if _, err := cubapi.EnsureTrigger(ctx, client, goclientnew.Trigger{
			...
			FunctionName:  t.fn,
			Arguments:     cubapi.Arguments(t.args),
			Warn:          true,
			Labels:        map[string]string{"Pack": autoscalePackLabel},
		}); err != nil {
	...
	flt, err := cubapi.EnsureFilter(ctx, client, goclientnew.Filter{
		SpaceID: ps.SpaceID,
		Slug:    autoscaleFilterSlug,
		From:    "Trigger",
		Where:   "Labels.Pack = '" + autoscalePackLabel + "'",
	})
	if err != nil {
		return fmt.Errorf("create filter %s: %w", autoscaleFilterSlug, err)
	}
	fprintln(out, fmt.Sprintf("Policy pack ready in %s.", policySpace))
	filterRef := policySpace + "/" + autoscaleFilterSlug
...
func newGuardrailsStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "status",
		Short: "List Units with autoscaling ApplyWarnings or ApplyGates",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := cub.Preflight(cmd.Context())
			if err != nil {
...
func guardrailFilterID(ctx context.Context, client *cubapi.Client, policySpace string) (string, error) {
	...
	flt, err := cubapi.ResolveFilter(ctx, client, ps.SpaceID, autoscaleFilterSlug)
	if err != nil {
		return "", err
	}
