...
const UNTRACKED_FILES = new Set([MANIFEST_PATH, 'data/live-bindings.json', 'data/cost-findings.json', 'confighub/registry/fleet-record.json']);
...
// Client registration state machine, recorded in the fleet record:
// unregistered -> registered (npm run oauth:register) -> rotated (rotate-auth,
// repeatable) -> revoked (decommission). Skipped states are typed BLOCK
// results at exit 0; every transition appends a recorded mutation.
async function rotateAuth() {
  ...
  if (!record) {
    return output({
      verdict: 'ERROR',
      reason: 'NO_RECORD',
      status: 'ROTATE_AUTH_BLOCKED_NO_RECORD',
      message: `${FLEET_RECORD_PATH} is required to record the client rotation.`,
    }, 1);
  }
  const client = record.oauthClient || {state: 'unregistered'};
  if (client.state === 'unregistered' || !client.state) {
    return output({
      verdict: 'BLOCK',
      reason: 'CLIENT_UNREGISTERED',
      status: 'ROTATE_AUTH_BLOCKED_UNREGISTERED',
      clientState: 'unregistered',
      message: 'The fleet record has no registered browser client for this app; rotation would skip the registered state.',
      nextGate: 'Run npm run oauth:register to create and record the client, then rerun rotate-auth with the replacement client id.',
    }, 0);
  }
  if (client.state === 'revoked') {
    return output({
      ...
      status: 'ROTATE_AUTH_BLOCKED_REVOKED',
      clientState: 'revoked',
      message: 'The recorded client registration was revoked at decommission; there is nothing to rotate.',
      nextGate: 'Run node lifecycle.mjs rollback --json to restore the pre-decommission record, or register again with npm run oauth:register.',
    }, 0);
  }
  if (!newClientId) {
    return output({
  ...
  const recordedClientId = client.clientId || auth.currentClientId || '';
  if (recordedClientId && recordedClientId === newClientId) {
    return output({
      verdict: 'BLOCK',
      reason: 'SAME_CLIENT',
  ...
  if (recordedClientId) {
    previous.push({clientId: recordedClientId, rotatedOutAt: rotatedAt});
  }
  ...
  await updateFleetRecord(rec => {
    const target = rec.oauthClient || (rec.oauthClient = {redirectUris: [], mutations: []});
    target.state = 'rotated';
    target.clientId = newClientId;
    target.lastRotatedAt = rotatedAt;
    target.mutations = [...(Array.isArray(target.mutations) ? target.mutations : []), {
      kind: 'rotate',
      from: recordedClientId,
      to: newClientId,
      at: rotatedAt,
    }];
    rec.lastLifecycleEvent = {command: 'rotate-auth', at: rotatedAt};
  });
  ...
  return output({
    ...
    previousClientId: recordedClientId,
    checklist: [
...
async function registry() {
  ...
  if (to === 'LIVE') {
    liveBindings = await liveBindingReadiness();
    if (liveBindings.status !== 'LIVE_BINDINGS_READY') {
      await updateFleetRecord(rec => {
        rec.bindings = {...liveBindings, checkedAt: nowIso()};
        rec.lastLifecycleEvent = {command: 'registry-live-blocked', at: nowIso()};
      });
      return output({
        verdict: 'BLOCK',
  ...
  await updateFleetRecord(rec => {
    ...
    if (liveBindings) rec.bindings = {...liveBindings, checkedAt: entry.timestamp};
...
async function decommission() {
  ...
  await updateFleetRecord(rec => {
    ...
    // Decommission leaves a client record: a registered or rotated client
    // registration becomes revoked, with the revocation recorded as a mutation.
    const client = rec.oauthClient;
    if (client && client.state && client.state !== 'unregistered' && client.state !== 'revoked') {
      client.state = 'revoked';
      client.revokedAt = at;
      client.mutations = [...(Array.isArray(client.mutations) ? client.mutations : []), {
        kind: 'revoke',
    ...
    clientState = client && client.state ? client.state : '';
  ...
  return output({
    ...
    fleetIndexUnit: record.fleetIndex ? record.fleetIndex.unitRef || '' : '',
    nextGate: 'Apply the RETIRED fleet-record Unit change through the governed path, delete the browser client through the supported ConfigHub path, remove delete gates deliberately, then retire the deployment. Run node lifecycle.mjs rollback --json to reverse.',
  });
}
