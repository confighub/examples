#!/usr/bin/env bash
# verify.sh — Confirm the sec-scanner guardrails installed by setup.sh
#
# Read-only. Checks that:
#   1. The policy Space holds the three image guardrail Triggers (Warn mode)
...
for trigger in valid-schemas no-latest-tag no-critical-cves; do
