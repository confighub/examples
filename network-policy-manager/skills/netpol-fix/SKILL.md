...
The config-as-data fix path for NetworkPolicy: close a coverage gap or restore intended connectivity by **editing the source of record**, so the cluster converges through the normal release pipeline with no drift. Every write is **dry-run by default**; nothing is written until you re-run with `--commit` and a `--change-desc`, and even then the Unit is created/edited but **not published**.
...
Allowed: the dry-run/commit writes above (always with `--change-desc` on commit) plus read commands for context. Not allowed: publishing a Release, `kubectl` mutation, bypassing gates.

## References

- `cub-netpol default-deny --help`, `cub-netpol allow --help`, `cub-netpol allow-from-links --help`, `cub-netpol fix --help`.
- Companion skills: **netpol-audit**, **netpol-findings**, **netpol-fleet**, **netpol-guardrails**, **release-publish**.
