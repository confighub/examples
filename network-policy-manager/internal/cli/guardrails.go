...
const (
	defaultNetpolPolicySpace = "netpol-policy"
	netpolFilterSlug         = "netpol-guardrails"
	netpolPackLabel          = "netpol-guardrails"
	// findingAnnotation is written by `guardrails annotate` onto Units the
	// analyzers flag, and read by the coverage Trigger (annotate-then-validate).
	findingAnnotation = "netpol.confighub.com/finding"
)
...
var guardrailTriggers = []guardrailTrigger{
	{"netpol-no-allow-all-ingress", "Warns on a NetworkPolicy ingress rule with an empty `from` (admits all sources). Fix: enumerate the allowed sources.", celNoAllowAllIngress},
	{"netpol-no-wide-cidr-egress", "Warns on an egress rule permitting 0.0.0.0/0. Fix: restrict the CIDR and exclude the cloud-metadata IP.", celNoWideCidrEgress},
	{"netpol-coverage-finding", "Warns while a netpol.confighub.com/finding annotation is present (set by `cub-netpol guardrails annotate`). Fix: close the coverage gap, then re-run annotate.", celNoCoverageFinding},
}
...
modified. Narrow which Spaces get wired with --where-space.`,
	...
	cmd.Flags().StringVar(&policySpace, "policy-space", defaultNetpolPolicySpace, "Space that holds the guardrail Triggers and Filter")
...
func buildGuardrailsPlan(ctx context.Context, client *cubapi.Client, policySpace, whereSpace string) (guardrailsPlan, error) {
	plan := guardrailsPlan{PolicySpace: policySpace, Filter: policySpace + "/" + netpolFilterSlug}
...
func executeGuardrails(cmd *cobra.Command, client *cubapi.Client, policySpace string, plan guardrailsPlan) error {
	...
	ps, err := cubapi.EnsureSpace(ctx, client, goclientnew.Space{
		Slug:   policySpace,
		Labels: map[string]string{"app": "network-policy-manager", "role": "policy"},
	})
	if err != nil {
	...
	for _, t := range guardrailTriggers {
		if _, err := cubapi.EnsureTrigger(ctx, client, goclientnew.Trigger{
			...
			Warn:          true,
			Labels:        map[string]string{"Pack": netpolPackLabel},
		}); err != nil {
	...
	flt, err := cubapi.EnsureFilter(ctx, client, goclientnew.Filter{
		SpaceID: ps.SpaceID,
		Slug:    netpolFilterSlug,
		From:    "Trigger",
		Where:   "Labels.Pack = '" + netpolPackLabel + "'",
	})
	if err != nil {
		return fmt.Errorf("create filter %s: %w", netpolFilterSlug, err)
	}
	fprintln(out, fmt.Sprintf("Policy pack ready in %s.", policySpace))

	filterRef := policySpace + "/" + netpolFilterSlug
...
func newGuardrailsStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "status",
		Short: "List Units with NetworkPolicy ApplyWarnings or ApplyGates",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := cub.Preflight(cmd.Context())
			if err != nil {
...
finding, so its annotation is removed). Dry run unless --commit --change-desc.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, key := range order {
				...
				if !dryRun {
					err := annotateUnit(cmd.Context(), client, spaceID, r.Unit, r.Analyzer, changeDesc)
					r.OK = err == nil
					if err != nil {
						r.Error = err.Error()
					}
...
func guardrailFilterID(ctx context.Context, client *cubapi.Client, policySpace string) (string, error) {
	...
	flt, err := cubapi.ResolveFilter(ctx, client, ps.SpaceID, netpolFilterSlug)
	if err != nil {
		return "", err
	}
