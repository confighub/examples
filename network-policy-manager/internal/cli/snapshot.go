...
Coverage gaps ("which namespaces/workloads have no policy?") are reported by the
'coverage' command in a later milestone; this is raw inventory only.`,
		Args: cobra.NoArgs,
...
func buildSnapshotReport(snap *snapshot.Snapshot) snapshotReport {
	...
	// NetworkPolicy-relevant config (i.e. appear in snap.Clusters); this
	// naturally excludes canonical Spaces.
...
func printSnapshotTable(cmd *cobra.Command, r snapshotReport) {
	...
	fmt.Fprintln(tw, "CLUSTER\tNETPOL\tNS\tWORKLOADS\tSVC\tUNITS\tGATED\tUNAPPLIED")
