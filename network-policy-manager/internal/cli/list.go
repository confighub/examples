...
func newListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List NetworkPolicy-relevant resources across the fleet (explorer)",
		Long: `list enumerates the resources ConfigHub holds that the NetworkPolicy model
reasons about — NetworkPolicy, Namespace, pod-bearing workloads (Deployment,
StatefulSet, DaemonSet, ReplicaSet, Job, CronJob, Pod), and Service — with the
...
Filter with --kind, --cluster, and --namespace.`,
	...
	cmd.Flags().StringVar(&kindFilter, "kind", "", "filter by kind (NetworkPolicy, Namespace, Deployment, ... , Service)")
...
func buildResourceRows(snap *snapshot.Snapshot, kindFilter, clusterFilter, namespaceFilter string) []resourceRow {
	...
	for _, r := range snap.Resources {
		kind, name, namespace, ok := netpol.ResourceMeta(r.Doc)
		if !ok {
			continue
		}
