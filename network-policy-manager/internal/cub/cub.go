...
// Package cub is cub-netpol's gateway to ConfigHub. It builds an authenticated
...
func Client(ctx context.Context) (*cubapi.Client, error) {
	clientOnce.Do(func() {
		client, clientErr = cubapi.ResolveClient(ctx, cubapi.ClientOptions{UserAgent: "cub-netpol"})
	})
