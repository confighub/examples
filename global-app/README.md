# Global App Deployment — ARCHIVED

> **This example no longer works and is retained for reference only.**
>
> It is built on `cub unit apply` and the bridge worker protocol, both of
> which have been removed from ConfigHub. The scripts under `bin/` will fail.
>
> The example was also substantially out of date relative to the current
> component and variant model even before that change.
>
> Its documentation page at
> `docs.confighub.com/get-started/examples/global-app/` has been removed; the
> full text is inlined below so it is preserved alongside the scripts and
> base config.
>
