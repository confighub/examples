...
jq -e '.since == "87600h"' "$OUTPUT_DIR/summary-list-all.json" >/dev/null
...
jq -e '.since == "87600h"' "$OUTPUT_DIR/summary-list-kind-dev-prod.json" >/dev/null
...
jq -e '.text | contains("87600h")' "$OUTPUT_DIR/summary-slack-dry-run.json" >/dev/null
