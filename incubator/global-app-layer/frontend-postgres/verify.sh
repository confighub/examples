...
verify_impl() {
  ...
  assert_contains "${recipe_manifest_file}" 'name: global-app-us-staging-app'
  ...
  echo "All global-app-layer frontend-postgres checks passed."
}
...
if verify_output="$(verify_impl 2>&1)"; then
  ...
  jq -n \
    ...
    --arg prefix "${PREFIX}" \
    --arg summary "All global-app-layer frontend-postgres checks passed." \
    --argjson targetExpected "$(if [[ -n "${TARGET_REF:-}" ]]; then echo true; else echo false; fi)" \
