...
- expected anchors:
  - `.example == "global-app-layer-frontend-postgres"`
  - `.mutates == false`
  - `.spaces | length == 5`
  - `.components | length == 3`
  - `.recipeManifest.unit == "recipe-us-staging-app"`
...
### `cub unit get --space <prefix>-recipe-us-staging --json recipe-us-staging-app`

- mutates: no
- output shape: JSON object containing `Space`, `Unit`, and `UnitStatus`
- proves: the app-level recipe receipt exists in ConfigHub
- jq anchor:
  - `cub unit get --space <prefix>-recipe-us-staging --json recipe-us-staging-app | jq '.Unit | {slug: .Slug, revision: .HeadRevisionNum, labels: .Labels}'`

### `cub unit get --space <prefix>-deploy-cluster-a --json frontend-cluster-a`

- mutates: no
- output shape: JSON object containing `Space`, `Unit`, `UnitStatus`, and often `UpstreamUnit`
- proves:
  - the final frontend deployment variant exists
  - target binding is inspectable if present
- jq anchor:
  - `cub unit get --space <prefix>-deploy-cluster-a --json frontend-cluster-a | jq '.Unit | {slug: .Slug, upstreamUnitID: .UpstreamUnitID, targetID: .TargetID, revision: .HeadRevisionNum}'`
...
- jq anchor:
  - `cub unit list --space <prefix>-deploy-cluster-a --quiet --json | jq '.[] | {slug: .Unit.Slug, upstream: (.UpstreamUnit.Slug // null), status: .UnitStatus.Status}'`
...
- the final line `All global-app-layer frontend-postgres checks passed.`
