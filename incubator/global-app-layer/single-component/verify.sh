...
if verify_output="$(verify_impl 2>&1)"; then
  ...
  jq -n \
    ...
    --arg prefix "${PREFIX}" \
    --arg summary "All global-app-layer single-component checks passed." \
    --argjson targetExpected "$(if [[ -n "${DIRECT_TARGET_REF:-}" || -n "${FLUX_TARGET_REF:-}" || -n "${ARGO_TARGET_REF:-}" ]]; then echo true; else echo false; fi)" \
