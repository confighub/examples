#!/usr/bin/env bash
# Show field-level mutation routing for inventory-api in ConfigHub.
#
...
# ── decode unit data (`cub unit data` prints it as text) ─────────────
decode_unit_data() {
  local raw="$1"
...
if command -v "${CUB}" >/dev/null 2>&1; then
  RAW_DATA=$(${CUB} unit data --space "${SPACE}" inventory-api 2>/dev/null || true)
fi
...
echo "Field routes for inventory-api-${ENV}:"
echo "────────────────────────────────────────────────────────────────────────"
echo ""
