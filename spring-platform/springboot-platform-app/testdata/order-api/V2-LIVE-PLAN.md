...
- add a `v2` live path for the same `order-api` service
- prove the same three routed outcomes against real state

## Goal

Make `order-api` a real Spring Boot service that we can:
...
- `src/main/java/com.example.orderapi/OrderApiApplication.java`
- `src/main/java/com.example.orderapi/api/OrderApiController.java`
- `src/main/java/com.example.orderapi/api/OrderApiItem.java`
- `src/main/java/com.example.orderapi/api/OrderApiService.java`
- `src/test/java/com.example.orderapi/api/OrderApiControllerTest.java`
...
- one app identity for `order-api`
...
- create or sync a small `order-api` GitHub repo for the live app inputs
...
These should prove that `order-api` is callable before any ConfigHub or
...
- call `order-api` and get real JSON values back
