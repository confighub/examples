...
#
# Creates ConfigHub spaces and units for order-api across dev, stage, prod.
#
# Usage:
...
INFRA_SPACE="order-api-infra"
DEPLOY_NAMESPACE="order-api"
LOCAL_IMAGE="order-api:local"
...
- 1 unit per space: order-api (ConfigMap + Deployment + Service)
- labels: ExampleName=springboot-platform-app, App=order-api, Environment=<env>
EOF
  case "${mode}" in
    "real-targets")
      cat <<EOF
...
- 1 infra space: order-api-infra (server worker)
...
for env in "${ENVS[@]}"; do
  space="$(space_name "${env}")"
  yaml_file="${SCRIPT_DIR}/confighub/order-api-${env}.yaml"
  ...
  if ${CUB} unit get --space "${space}" order-api >/dev/null 2>&1; then
    echo "y" | ${CUB} unit delete --space "${space}" order-api >/dev/null 2>&1 || true
  fi

  ${CUB} unit create --space "${space}" order-api "${yaml_file}" \
    --label "ExampleName=${EXAMPLE_LABEL}" \
    --label "App=order-api" \
    --label "Environment=${env}" \
    --label "Component=backend" \
    --quiet
  echo "  Created unit: ${space}/order-api"
done
...
case "${MODE}" in
  "real-targets")
    ...
    ${CUB} function do --space order-api-prod \
      --unit order-api \
    ...
    ${CUB} function do --space order-api-prod \
      ...
      --quiet \
      set-image order-api "${LOCAL_IMAGE}"
    ...
    ${CUB} unit set-target "${WORKER_SPACE}/${K8S_TARGET}" \
      ...
      --unit order-api \
      --quiet
    echo "  Bound: order-api-prod/order-api -> ${WORKER_SPACE}/${K8S_TARGET}"
    echo "  Done."
    echo ""

    echo "Phase 6: Applying prod unit to cluster..."
    ${CUB} unit apply --space order-api-prod order-api --quiet
    echo "  Applied: order-api-prod/order-api"
    echo ""
    echo "  Waiting for deployment to be ready..."
    kubectl rollout status deployment/order-api -n "${DEPLOY_NAMESPACE}" --timeout=120s || {
      echo "  warning: Deployment not ready within timeout. Check:" >&2
      echo "    kubectl get pods -n ${DEPLOY_NAMESPACE}" >&2
      echo "    kubectl describe deployment order-api -n ${DEPLOY_NAMESPACE}" >&2
    }
    echo "  Done."
    echo ""
    ;;
  ...
  "noop-targets")
    ...
    for env in "${ENVS[@]}"; do
      ...
      ${CUB} unit set-target "${env}" --space "${space}" --unit order-api --quiet
      echo "  Bound unit: ${space}/order-api -> ${env}"
    done
    ...
    for env in "${ENVS[@]}"; do
      space="$(space_name "${env}")"
      ${CUB} unit apply --space "${space}" order-api --quiet
      echo "  Applied: ${space}/order-api"
    done
...
echo "  ${CUB} unit get --space order-api-prod --json order-api"
