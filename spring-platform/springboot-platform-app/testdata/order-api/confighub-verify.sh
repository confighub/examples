...
# Check each env space has the order-api unit
for env in "${ENVS[@]}"; do
  space="order-api-${env}"
  unit_json=$(${CUB} unit get --space "${space}" --json order-api 2>&1) || {
    echo "FAIL: unit order-api not found in space ${space}" >&2
    errors=$((errors + 1))
    continue
  }
  ...
  if [[ "${data_len}" -lt 1 ]]; then
    echo "FAIL: unit ${space}/order-api has no data" >&2
    errors=$((errors + 1))
  else
    echo "ok: ${space}/order-api has data (${data_len} bytes)"
  fi
  ...
  if [[ "${MODE}" != "confighub-only" ]]; then
    ...
    if [[ "${status}" != "Ready" && "${status}" != "Unknown" ]]; then
      echo "FAIL: ${space}/order-api status is ${status}, expected Ready" >&2
      errors=$((errors + 1))
    else
      echo "ok: ${space}/order-api status=${status} sync=${sync}"
    fi
  fi
done
...
case "${MODE}" in
  "noop-targets")
    ...
    infra_json=$(${CUB} space list --where "Labels.ExampleName = '${EXAMPLE_LABEL}'" --json \
      | jq '[.[] | select(.Space.Slug == "order-api-infra")]')
    infra_count=$(echo "${infra_json}" | jq 'length')
    if [[ "${infra_count}" -lt 1 ]]; then
      echo "FAIL: infra space order-api-infra not found" >&2
      errors=$((errors + 1))
    else
      echo "ok: infra space order-api-infra exists"
    fi
  ...
  "real-targets")
    ...
    if ! command -v kubectl >/dev/null 2>&1; then
      ...
      DEPLOYMENT_STATUS=$(kubectl get deployment order-api -n "${DEPLOY_NAMESPACE}" \
        -o jsonpath='{.status.conditions[?(@.type=="Available")].status}' 2>/dev/null || echo "NotFound")
      if [[ "${DEPLOYMENT_STATUS}" == "True" ]]; then
        echo "ok: deployment 'order-api' is Available"
      elif [[ "${DEPLOYMENT_STATUS}" == "NotFound" ]]; then
        echo "FAIL: deployment 'order-api' not found" >&2
        errors=$((errors + 1))
      else
        echo "FAIL: deployment 'order-api' is not Available (status: ${DEPLOYMENT_STATUS})" >&2
        errors=$((errors + 1))
      fi

      # Check pods are running
      RUNNING_PODS=$(kubectl get pods -n "${DEPLOY_NAMESPACE}" -l app=order-api \
        --field-selector=status.phase=Running -o name 2>/dev/null | wc -l | tr -d ' ')
      if [[ "${RUNNING_PODS}" -gt 0 ]]; then
        echo "ok: ${RUNNING_PODS} pod(s) running"
      else
        echo "FAIL: no running pods found" >&2
        kubectl get pods -n "${DEPLOY_NAMESPACE}" -l app=order-api 2>/dev/null | sed 's/^/    /' || true
        errors=$((errors + 1))
      fi

      # Check prod unit is bound to a real target
      PROD_TARGET=$(${CUB} unit get --space order-api-prod --json order-api 2>/dev/null | \
        jq -r '.UnitStatus.TargetRef // "none"')
      if [[ "${PROD_TARGET}" != "none" && "${PROD_TARGET}" != "null" && -n "${PROD_TARGET}" ]]; then
        echo "ok: order-api-prod/order-api bound to target: ${PROD_TARGET}"
        ...
        echo "FAIL: order-api-prod/order-api not bound to a target" >&2
        errors=$((errors + 1))
      fi
    fi
    ;;
