#!/usr/bin/env bash
# Compare order-api config across dev, stage, prod in ConfigHub.
#
...
Compares order-api operational config across dev, stage, and prod.
...
Commands used:
- cub unit get --space <space> --data-only --json order-api
EOF
}
...
for env in "${ENVS[@]}"; do
  ...
  if command -v "${CUB}" >/dev/null 2>&1; then
    raw=$(${CUB} unit get --space "order-api-${env}" --data-only --json order-api 2>/dev/null || true)
  fi
