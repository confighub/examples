...
1. APP NAME EXTRACTION
   Input:  spring.application.name: order-api
   Output: metadata.name: order-api (in all manifests)
...
spring.application.name: order-api
  → deployment.yaml: metadata.name = order-api
  → deployment.yaml: spec.selector.matchLabels.app.kubernetes.io/name = order-api
  → configmap.yaml: metadata.name = order-api-config
  → service.yaml: metadata.name = order-api
...
Example mutation:
  cub function do --space order-api-prod --unit order-api \
    set-env order-api FEATURE_INVENTORY_RESERVATIONMODE=optimistic
