...
func newPromoteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "promote",
		Short: "Upgrade downstream Units to their upstream head (dry-run unless --commit)",
...
observability change (a ServiceMonitor, a sidecar) authored in a base Space flows
to the environment/region Spaces cloned from it, keeping local customizations.

Scope with --where or a label shorthand. Dry run unless --commit --change-desc;
never bypasses ApplyGates.`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			changeDesc, dryRun, err := commit.Validate("promote observability Units from upstream")
			if err != nil {
				return err
			}
