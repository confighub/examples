...
const (
	defaultObsPolicySpace = "observability-policy"
	obsFilterSlug         = "observability-guardrails"
	obsPackLabel          = "observability-guardrails"
	// coverageAnnotation is written by `guardrails annotate` onto each uncovered
	// metrics-Service Unit, and read by the coverage Trigger (annotate-then-validate
	// — coverage is a cross-Unit property a per-Unit vet-cel can't compute).
...
var guardrailTriggers = []guardrailTrigger{
	{"servicemonitor-coverage",
		"Warns while an observability.confighub.com/coverage annotation is present (set by `cub-observability guardrails annotate`) — a metrics Service with no ServiceMonitor. Fix: `cub-observability ensure-servicemonitor`, then re-run annotate.",
		celNoCoverageFinding},
}
...
func newGuardrailsInstallCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&policySpace, "policy-space", defaultObsPolicySpace, "Space that holds the guardrail Triggers and Filter")
...
func buildGuardrailsPlan(ctx context.Context, client *cubapi.Client, policySpace, whereSpace string) (guardrailsPlan, error) {
	plan := guardrailsPlan{PolicySpace: policySpace, Filter: policySpace + "/" + obsFilterSlug}
...
func executeGuardrails(cmd *cobra.Command, client *cubapi.Client, policySpace string, plan guardrailsPlan) error {
	...
	ps, err := cubapi.EnsureSpace(ctx, client, goclientnew.Space{
		Slug:   policySpace,
		Labels: map[string]string{"app": "observability-manager", "role": "policy"},
	})
	if err != nil {
	...
	for _, t := range guardrailTriggers {
		if _, err := cubapi.EnsureTrigger(ctx, client, goclientnew.Trigger{
			...
			Warn:          true,
			Labels:        map[string]string{"Pack": obsPackLabel},
		}); err != nil {
	...
	flt, err := cubapi.EnsureFilter(ctx, client, goclientnew.Filter{
		...
		Slug:    obsFilterSlug,
		From:    "Trigger",
		Where:   "Labels.Pack = '" + obsPackLabel + "'",
	})
	if err != nil {
		return fmt.Errorf("create filter %s: %w", obsFilterSlug, err)
	}
	fprintln(out, fmt.Sprintf("Policy pack ready in %s.", policySpace))
	filterRef := policySpace + "/" + obsFilterSlug
...
func newGuardrailsStatusCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "status",
		Short: "List Units with observability ApplyWarnings or ApplyGates",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			client, err := cub.Preflight(cmd.Context())
			if err != nil {
...
Re-run after adding ServiceMonitors. Dry run unless --commit --change-desc.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, r := range observability.AnalyzeCoverage(snap.Clusters) {
				...
				if !dryRun {
					err := annotateUnit(cmd.Context(), client, r.SpaceID, r.UnitSlug, "uncovered", changeDesc)
					ar.OK = err == nil
					if err != nil {
						ar.Error = err.Error()
					}
	...
	_, err := cubapi.InvokeFunction(ctx, client,
		api.FunctionInvocation{
			...
			Arguments: []api.FunctionArgument{
				{ParameterName: "annotation-key", Value: coverageAnnotation},
...
func guardrailFilterID(ctx context.Context, client *cubapi.Client, policySpace string) (string, error) {
	...
	flt, err := cubapi.ResolveFilter(ctx, client, ps.SpaceID, obsFilterSlug)
	if err != nil {
		return "", err
	}
