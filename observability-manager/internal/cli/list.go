...
func newListCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List ServiceMonitors and Services across the fleet (explorer)",
		Long: `list enumerates the ServiceMonitors and Services ConfigHub holds, with the
cluster, Space, and Unit each came from. Canonical base/policy definitions are
included and flagged.

Filter with --kind (ServiceMonitor, Service), --cluster, and --namespace.`,
		Args: cobra.NoArgs,
	...
	cmd.Flags().StringVar(&kindFilter, "kind", "", "filter by kind (ServiceMonitor, Service)")
...
func buildResourceRows(snap *snapshot.Snapshot, kindFilter, clusterFilter, namespaceFilter string) []resourceRow {
	...
	for _, r := range snap.Resources {
		kind, name, namespace, ok := observability.ResourceMeta(r.Doc)
		if !ok || (kind != "ServiceMonitor" && kind != "Service") {
			continue
		}
		...
		rows = append(rows, resourceRow{
			Cluster: r.Origin.Cluster, Kind: kind, Name: name, Namespace: namespace,
			Space: r.Origin.Space, UnitSlug: r.Origin.UnitSlug, Canonical: r.Origin.Canonical,
		})
	}
