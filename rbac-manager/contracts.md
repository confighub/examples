...
- creates: 3 Warn=true guardrail Triggers (label `Pack=rbac-guardrails`) plus a
  ...
  `policy-guardrails`, override with `--policy-space SLUG`); and the 7
  parameterized set-yq edit Invocations (`rbac-add-verb`, `rbac-remove-verb`,
  `rbac-add-subject`, `rbac-remove-subject`, `rbac-set-rule`, `rbac-add-rule`,
  `rbac-remove-rule`) in the `rbac-edits` Space — the shared, declarative edit
  templates the app and the agent CLI invoke with parameters instead of
  compiling yq client-side
