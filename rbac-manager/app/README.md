...
```bash
cub oauthclient create rbac-manager --redirect-uri http://localhost:5180/
...
When you are done experimenting, remove the client: `cub oauthclient delete rbac-manager`.
...
npm test             # this app's own tests; the RBAC engine's live in ../../webkit
```
...
and snapshot machinery, config data access, and the RBAC engine — lives in
[`../../webkit`](../../webkit). What is left in `src/` is this app's own: the pages, the
persona editing invocations, and the friendly RBAC views.
