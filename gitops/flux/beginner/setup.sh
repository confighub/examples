...
DEV_SPACE="${GITOPS_FLUX_BEGINNER_DEV_SPACE:-gitops-flux-beginner-dev}"
PROD_SPACE="${GITOPS_FLUX_BEGINNER_PROD_SPACE:-gitops-flux-beginner-prod}"
EXPLAIN=0
EXPLAIN_JSON=0
...
if [[ "$EXPLAIN" -eq 1 ]]; then
  cat <<EOF_PLAN
This is a read-only setup plan for gitops/flux/beginner.
Nothing will be mutated.

Conceptual model (standard Flux kustomize-helm layout):

  clusters/dev/infrastructure.yaml  (Flux Kustomization) -> infrastructure/dev -> infrastructure/base (GitRepository source)
  clusters/dev/apps.yaml            (Flux Kustomization) -> apps/dev           -> apps/base (Deployment, Service)
  clusters/prod/... the same, pointed at apps/prod

This example will:
- render apps/dev with "kustomize build"
- render apps/prod with "kustomize build"
...
This example never bootstraps Flux against a live cluster. The clusters/
directory shows the same Kustomization shape "flux bootstrap" would create;
this example does not apply it anywhere.
EOF_PLAN
  exit 0
fi
