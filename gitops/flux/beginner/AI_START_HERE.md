...
Use this page when you want to drive `gitops/flux/beginner` safely with an
AI assistant.
...
```text
Read gitops/flux/beginner/AI_START_HERE.md and walk me through the demo.
...
This is a beginner-level Flux layout: one app, two environments (dev, prod),
using the standard `clusters/`, `infrastructure/`, `apps/` split. It renders
with Kustomize and uploads into ConfigHub. It does not create a cluster and
does not run `flux bootstrap`.
...
- creates or updates ConfigHub Space `gitops-flux-beginner-dev`
- creates or updates ConfigHub Space `gitops-flux-beginner-prod`

What this does not mutate:

- no live cluster, no Flux bootstrap or installation
...
This re-renders both app overlays and the infrastructure layer locally, and
checks the expected resources are present. It does not call ConfigHub, so it
will pass even if you skipped Stage 2.
