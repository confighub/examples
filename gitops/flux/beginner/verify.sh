...
if [[ "$name" != "gitops-flux-beginner" ]]; then
...
echo "==> Rendering the dev app overlay with kustomize build"
kustomize build "$SCRIPT_DIR/apps/dev" > "$VAR_DIR/rendered-dev.yaml"

echo "==> Rendering the prod app overlay with kustomize build"
kustomize build "$SCRIPT_DIR/apps/prod" > "$VAR_DIR/rendered-prod.yaml"

echo "==> Rendering the infrastructure layer for dev and prod"
kustomize build "$SCRIPT_DIR/infrastructure/dev" > "$VAR_DIR/rendered-infra-dev.yaml"
kustomize build "$SCRIPT_DIR/infrastructure/prod" > "$VAR_DIR/rendered-infra-prod.yaml"
...
echo "==> Checking infrastructure layer contains the shared GitRepository source"
grep -q "^kind: GitRepository\$" "$VAR_DIR/rendered-infra-dev.yaml"
grep -q "  name: apptique-examples\$" "$VAR_DIR/rendered-infra-dev.yaml"
grep -q "^kind: GitRepository\$" "$VAR_DIR/rendered-infra-prod.yaml"

echo "==> Checking clusters/ Kustomizations point at this example's paths"
grep -q "gitops/flux/beginner/apps/dev" "$SCRIPT_DIR/clusters/dev/apps.yaml"
grep -q "gitops/flux/beginner/apps/prod" "$SCRIPT_DIR/clusters/prod/apps.yaml"
grep -q "gitops/flux/beginner/infrastructure/dev" "$SCRIPT_DIR/clusters/dev/infrastructure.yaml"
grep -q "gitops/flux/beginner/infrastructure/prod" "$SCRIPT_DIR/clusters/prod/infrastructure.yaml"

echo "All gitops-flux-beginner checks passed."
