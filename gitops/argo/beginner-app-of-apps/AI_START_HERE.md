...
Use this page when you want to drive `gitops/argo/beginner-app-of-apps`
...
```text
Read gitops/argo/beginner-app-of-apps/AI_START_HERE.md and walk me through the demo.
...
This is a beginner-level Argo CD app-of-apps layout: one root Application,
one child Application per environment (dev, prod), and plain YAML per
environment. It collects each environment's YAML and uploads it into
ConfigHub. It does not create a cluster and does not install Argo CD.
...
- creates or updates ConfigHub Space `gitops-argo-beginner-aoa-dev`
- creates or updates ConfigHub Space `gitops-argo-beginner-aoa-prod`
...
This checks the root and child Applications, collects each environment's
YAML locally, and checks the expected resources are present. It does not call ConfigHub, so it will pass even if you skipped
Stage 2.
