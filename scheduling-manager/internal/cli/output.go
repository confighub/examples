...
// Unit `--where` predicate (Unit / Space.* / Target.* metadata). The label flags
// are convenience shorthands over --where.
...
// predicate compiles the flags into a single ConfigHub `where` expression. It is
// flat AND-only — no parentheses, no OR.
