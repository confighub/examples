...
2. **Install** with `--commit`: `cub-scheduling guardrails install --commit`. The Trigger is `Warn=true` (advisory). Promote it to blocking with `cub trigger update workload-toleration-needs-placement --space common --unwarn`.
