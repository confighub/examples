...
describe('evaluate — filter', () => {
  ...
  it('evaluates column-to-column comparison (drift idiom)', () => {
    ...
    const rows: Row[] = [
      { slug: 'drifted', headRevisionNum: 8, lastReleasedRevisionNum: 5 },
      { slug: 'synced', headRevisionNum: 5, lastReleasedRevisionNum: 5 },
      { slug: 'never', headRevisionNum: 3, lastReleasedRevisionNum: 0 },
    ];
    const q = 'SELECT slug FROM units WHERE headRevisionNum > lastReleasedRevisionNum';
