...
const UNITS: TableDef = {
  ...
  columns: {
    ...
    gates: { type: 'number' }, // count of validationErrors keys
    warnings: { type: 'number' }, // count of validationWarnings keys
  },
  mapPrefixes: {
    ...
    //   validationErrors['<space-slug>/<trigger-slug>/<function-name>']
    // (a legacy 2-part `<trigger-slug>/<function-name>` is also accepted), e.g.
    //   validationErrors['sec-demo-policy/no-critical-cves/vet-celexpr'] = true
    // true = the gate is failing and blocks apply. Pushes to `where` as
    // `ValidationErrors.<key> = true` (JSONB containment). ConfigHub regex-validates the
    ...
    validationErrors: { type: 'boolean', pushdown: { target: 'where', field: 'ValidationErrors' } },
    validationWarnings: { type: 'boolean', pushdown: { target: 'where', field: 'ValidationWarnings' } },
    ...
    // narrow the fetch; for an exact key use validationErrors['<full/key>'] (pushes).
    gate: { type: 'boolean' },
