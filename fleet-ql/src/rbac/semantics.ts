...
/**
 * A partial access query: any field left undefined is a wildcard (matches
 * anything). When verb AND resource are both set this is equivalent to
 * ruleMatches; the looseness lets the FQL grants materializer answer "who can
 * delete *" (resource omitted) or "who can * secrets" (verb omitted) from a
 * partial WHERE. Added for fleet-ql; not part of the original rbac-manager API.
 */
export interface PartialAccessQuery {
  verb?: string;
  resource?: string;
  apiGroup?: string;
  name?: string;
...
export function accessMatches(rule: PolicyRule, q: PartialAccessQuery): boolean {
  if (rule.resources.length === 0) return false; // nonResourceURL-only rule
  if (q.verb !== undefined && !verbMatches(rule.verbs, q.verb)) return false;
  if (q.apiGroup !== undefined && !groupMatches(rule.apiGroups, q.apiGroup)) return false;
  if (q.resource !== undefined && !resourceMatches(rule.resources, q.resource)) return false;
  if (!nameMatches(rule.resourceNames, q.name)) return false;
  return true;
}
