...
# Source this from any script under global-app-layer/e2e.

E2E_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LAYER_DIR="$(dirname "${E2E_DIR}")"
REPO_ROOT="$(cd "${LAYER_DIR}/.." && pwd)"
# The kind cluster that e2e/bin/create-cluster writes. Set E2E_KUBECONFIG to
# use another cluster.
KUBECONFIG_PATH="${E2E_KUBECONFIG:-${E2E_DIR}/var/gitops-import-argo.kubeconfig}"
...
require_kubeconfig() {
  if [[ ! -f "${KUBECONFIG_PATH}" ]]; then
    ...
    echo "Run global-app-layer/e2e/bin/create-cluster + install-argocd + install-worker first." >&2
    exit 1
  fi
...
require_infrastructure() {
  ...
  if [[ "${worker_condition}" != "Ready" ]]; then
    ...
    echo "Run: CUB_SPACE=${WORKER_SPACE} global-app-layer/e2e/bin/install-worker" >&2
    exit 1
  fi
}
