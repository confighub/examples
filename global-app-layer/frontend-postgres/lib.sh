...
source_yaml_for() {
  local component="$1"
  case "${component}" in
    frontend) echo "${SCRIPT_DIR}/../baseconfig/frontend.yaml" ;;
    postgres) echo "${SCRIPT_DIR}/../baseconfig/postgres.yaml" ;;
    *)
...
- backend dependency stub: $(deploy_space)/${DEPLOY_STUB_UNIT}
...
- postgres region: set REGION=${REGION_VALUE}
...
- backend dependency stub: create backend stub and set its namespace

Recipe manifest:
- $(recipe_space)/${RECIPE_MANIFEST_UNIT}
- source template: ${RECIPE_BASE_TEMPLATE}
- rendered output: ${STATE_DIR}/recipe-us-staging-app.rendered.yaml
...
show_setup_plan_json() {
  ...
  jq -n \
    ...
    --arg stubSource "${BACKEND_STUB_YAML}" \
    ...
    --arg manifestRendered "${STATE_DIR}/recipe-us-staging-app.rendered.yaml" \
    '{
      example: $example,
      mode: "setup-plan",
      mutates: false,
