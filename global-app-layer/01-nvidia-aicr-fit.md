...
ConfigHub now has a small layered example package in `examples/global-app-layer/`, but it still needs a bigger GPU-style example to match the AICR story more directly.
...
The `examples/global-app-layer/` package is the beginning of this story.
...
- `gpu-eks-h100-training/`: a domain-shaped GPU recipe where `gpu-operator` and `nvidia-device-plugin` share platform, accelerator, OS, and intent layers
...
1. Keep `examples/global-app-layer/single-component/` as the smallest worked proof.
2. Keep `examples/global-app-layer/frontend-postgres/` as the small shared-layer proof.
3. Keep `examples/global-app-layer/realistic-app/` as the fuller app-level proof.
4. Keep `examples/global-app-layer/gpu-eks-h100-training/` as the first multi-component GPU recipe proof.
